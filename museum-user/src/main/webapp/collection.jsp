...
<body>
   ...
   <div class="box-type">
       <ul class="type">
           ...
           <c:forEach items="${typeList}" var="type">
               <li class="li"><a href="${pageContext.request.contextPath}/collections/getByTypePage?type=${type.typeName}">${type.typeName}</a></li>
           </c:forEach>
       </ul>
       <ul class="collection">
           <c:forEach items="${collections}" var="collection">
               <li><a href="${pageContext.request.contextPath}/collections/getCollectionById?collectionId=${collection.collectionId}">
                   <img src="${collection.photo}" alt="">
               </a>
                   <a href="${pageContext.request.contextPath}/collections/getCollectionById?collectionId=${collection.collectionId}"><div class="content">${collection.profile}</div></a>
                   <a href="${pageContext.request.contextPath}/collections/getCollectionById?collectionId=${collection.collectionId}"><p>${collection.collectionName}</p></a>
               </li>
           </c:forEach>
       </ul>
   </div>
   <div class="page">
       <ul>
           <li class="a"><a href="${pageContext.request.contextPath}/collections/getByTypePage?currentPage=1&type=${type}">首页</a></li>
           <c:if test="${currentPage-1>=1}">
               <li class="b"><a href="${pageContext.request.contextPath}/collections/getByTypePage?currentPage=${currentPage-1}&type=${type}">&lt;</a></li>
           </c:if>
           <li class="c">${currentPage}</li>
           <c:if test="${currentPage+1 <= total}">
               <li class="d"><a href="${pageContext.request.contextPath}/collections/getByTypePage?currentPage=${currentPage+1}&type=${type}">&gt;</a></li>
           </c:if>
           <li><a href="${pageContext.request.contextPath}/announcements/getAnnouncementsByPage?currentPage=${total}&type=${type}">尾页</a></li>
       </ul>
   </div>
   </div>
</body>
